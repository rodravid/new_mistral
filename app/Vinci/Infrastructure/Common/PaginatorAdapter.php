...
class PaginatorAdapter
{
    ...
    public function makeRaw(AbstractQuery $query, $perPage, $currentPage = 1, $currentPath = '/', $fetchJoinCollection = true)
    {
        $this->query($query)
            ->skip($this->getSkipAmountRaw($currentPage, $perPage))
            ->take($perPage);

        return $this->convertToLaravelPaginatorRaw(
            $this->getDoctrinePaginator($fetchJoinCollection),
            $perPage,
            $currentPage,
            $currentPath
    ...
    protected function getSkipAmountRaw($currentPage, $perPage)
    {
    ...
    protected function convertToLaravelPaginatorRaw(DoctrinePaginator $doctrinePaginator, $perPage, $currentPage = 1, $path = '/')
    {
        $results = $this->getResults($doctrinePaginator);
