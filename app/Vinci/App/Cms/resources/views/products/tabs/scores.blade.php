<div class="tab-pane {{ currentTabActive('#productScores') }}" id="productScores">
    <div class="row">
        <div class="col-xs-12">
            <div class="row">
                <div class="col-xs-12">
                    <button type="button" id="btnAddScore" class="btn btn-success" title="Adicionar uva"><i class="fa fa-plus-circle"></i> <span class="hidden-xs">Adicionar</span></button>
                </div>
            </div>
            <div class="row">
                <div class="col-xs-12" id="boxGrapes">
                    @if($wineScores)
                        @foreach($wineScores as $i => $score)
                            <div class="row item">
                                <div class="col-xs-6 col-sm-6">
                                    <div class="form-group">
                                        <label>Uva</label>
                                        {!! Form::text('grapes[' . $i . '][name]', $score['name'], ['class' => 'form-control', 'readonly']) !!}
                                        {!! Form::hidden('grapes[' . $i . '][id]', $score['id'], ['class' => 'txtGrapeId']) !!}
                                    </div>
                                </div>
                                <div class="col-xs-3 col-sm-4">
                                    <div class="form-group">
                                        <label>Peso %</label>
                                        {!! Form::text('grapes[' . $i . '][weight]', $score['weight'], ['class' => 'form-control']) !!}
                                    </div>
                                </div>
